...
class Deployer
  ...
  def initialize(subscription_id, resource_group, resource_group_location)
    raise ArgumentError.new("Missing template file 'azuredeploy.json' in current directory.") unless File.exist?('azuredeploy.json')
    raise ArgumentError.new("Missing parameters file 'azuredeploy.parameters.json' in current directory.") unless File.exist?('azuredeploy.parameters.json')
  ...
  def deploy
    ...
    template = File.read(File.expand_path(File.join(__dir__, 'azuredeploy.json')))
    ...
    deploy_params = File.read(File.expand_path(File.join(__dir__, 'azuredeploy.parameters.json')))
